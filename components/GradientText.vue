...
<template>
  <Motion
    :tag="showBorder ? 'div' : 'span'"
    :class="[
      'relative font-medium transition-shadow duration-500 cursor-pointer overflow-visible',
      showBorder ? 'mx-auto flex max-w-fit flex-row items-center justify-center rounded-[1.25rem] py-1 px-2' : 'inline',
      className
    ]"
  ...
  >
    ...
    <Motion
      :tag="showBorder ? 'div' : 'span'"
      :class="[
        'z-2 relative bg-clip-text text-transparent',
        showBorder ? 'inline-block' : 'inline'
      ]"
      :style="{
        ...gradientStyle,
        backgroundPosition,
